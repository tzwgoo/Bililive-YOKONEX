...
- 客户端静默连接独立管理服务器，不增加本地页面或操作入口
- 管理后台提供管理员登录、在线客户端、已连接设备和操作记录
- 支持远程发送固定 `commandId`、播放客户端已上报波形、停止输出和断开设备
- 管理服务器同时支持 `HTTP/WS` 和 `HTTPS/WSS` 部署
...
| [设备后台管理](docs/设备后台管理.md) | 管理服务器配置、启动和客户端接入 |
